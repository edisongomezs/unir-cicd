...
test-api:
	IF NOT EXIST calc-test-api docker network create calc-test-api
...
test-e2e:
	IF EXIST calc-test-e2e docker network rm calc-test-e2e
...
start-sonar-server:
	IF NOT EXIST calc-sonar docker network create calc-sonar
